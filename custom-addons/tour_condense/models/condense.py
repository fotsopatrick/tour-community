...
Demande de Patrick (29/07, tâches 656 et 657) : « les textes, mes users vont
fuir ». Tout texte long rendu par un agent (compte rendu d'atelier, réponse,
étude Braignak, avis de débat, note du Dépôt…) doit s'afficher PAR DÉFAUT
comme un résumé court, compréhensible par un enfant de six ans. Le texte
d'origine reste entier, dans un onglet « Détail ». Rien n'est supprimé.
...
CATALOGUE = [
    ...
    ("braignak.etude", "observations", 400),
    ("braignak.etude", "justification", 280),
...
class CondenseEngine(models.AbstractModel):
    ...
    # ------------------------------------------------------------------
    # Le recours IA (DeepSeek, clé du Coffre) — seulement si la coupe
    ...
    def _resumer_ia(self, texte, bref=False):
        ...
        if "vault.secret" in self.env:
            sec = self.env["vault.secret"].sudo().search(
                [("name", "=", "deepseek-api-key")], limit=1)
            if sec:
                try:
                    cle = sec.lire_secret()
