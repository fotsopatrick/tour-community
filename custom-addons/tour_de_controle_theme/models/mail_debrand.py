# -*- coding: utf-8 -*-
"""Réécrire les courriels que la tour envoie réellement.
...
# qui a cree la fiche. Ce n'est pas la meme : un compte cree par un script
# annoncait « Copilote vous invite », ce qui n'a aucun sens pour qui le recoit.
...
class MailDebrand(models.AbstractModel):
    ...
    def appliquer(self):
        ...
        # Écrire le gabarit ne suffit pas : `name`, `subject` et `body_html`
        # sont des champs TRADUITS (stockés par langue). Le `write` de base ne
        # pose que la valeur par défaut (en_US) — la traduction française,
        # elle, reste celle d'Odoo (« Tu es invité(e) à rejoindre notre espace
        # de travail »), et c'est elle qui part à un destinataire francophone.
        # Constaté le 31/07 : l'invitation envoyée était bien la version Odoo,
        # pas la nôtre. On écrit donc aussi la version française.
