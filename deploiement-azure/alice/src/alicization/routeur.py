...
def call_model(messages, temperature=0.2, max_tokens=256):
    ...
    #  - sans niveau de raisonnement bas, gpt-5 consomme tout le budget en
    #    tokens de raisonnement et ne rend JAMAIS de contenu (28/08) →
    #    on force reasoning_effort=low pour obtenir une réponse visible.
    if any(nom in modele.lower() for nom in ("gpt-5", "o1", "o3", "o4")):
        payload["max_completion_tokens"] = max_tokens
        if "gpt-5" in modele.lower():
